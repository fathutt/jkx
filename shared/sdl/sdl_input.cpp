...
// ConvertUTF32ToExpectedCharset used to be here: two hundred lines mapping code
// points onto single bytes of Windows-1251, because everything downstream - the
// edit fields, the console buffer, the font - was one byte per character. All
// three are UTF-8 now, so a code point travels from the keyboard to the atlas
// without being asked which codepage it belongs to.
/*
===============
IN_ProcessEvents
===============
				...
				if( lastKeyDown != A_CONSOLE )
				{
					...
					while( *c )
					{
						...
						if( utf32 != 0 )
						{
							...
							{
								// The code point, not a byte of Windows-1251.
								// SE_CHAR carries an int and always did; what it
								// carried was squeezed through a 256 entry table
								// on the way, and everything outside that page -
								// which is most of the world - arrived as a
								// question mark or as nothing.
								Sys_QueEvent( 0, SE_CHAR, (int)utf32, 0, 0, NULL );
							}
						}
					}
				}
