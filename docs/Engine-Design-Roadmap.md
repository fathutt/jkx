...
| 2.4 | SP World Effects API поверх GPU-погоды: `IsOutside`, `IsOutsideCausingPain`, `IsShaking`, `GetWindVector`, `GetChanceOfSaberFizz`. Нужен point-in-brush тест, которого в рантайме нет | 2–3 нед | сделано: все восемь `WE_*` заведены через `sv_game.cpp`, ветер и тряска отвечают по зоне, а не глобально; point-in-brush закрыт точечным кэшем `COutside`. Проверяется стендом (`windzone`/`windat`). На ретейловых картах с несколькими зонами не проверялось |
