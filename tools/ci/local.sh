...
# that does not is printed. On the vertex-buffer path that square is NOT DRAWN -
# and not because the deform carries it off screen, because a bulge height of
# ZERO, which cannot move anything, leaves it just as absent. So it is the draw
# being skipped, and the shape of that is familiar: a pipeline that does not
# exist makes vk_bind_pipeline skip in silence, which is what hid the sky
# cubemap. That is where to look next.
stage_smokedeform() {
    JKX_SMOKE_DEFORM=1 \
