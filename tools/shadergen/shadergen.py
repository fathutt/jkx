...
def load_manifest(path: Path) -> tuple[dict, list[Variant]]:
    ...
    for suffix, stage in standalone.get("extensions", {}).items():
        for src in sorted(glsl_dir.glob(f"*{suffix}")):
            # The "_spv" suffix matches the identifiers the old generated C
            # used, so the hand-written binding lines in vk_shaders.cpp keep
            # working verbatim. Minimising the diff of a mechanical change is
            # worth more here than tidier names.
            name = f"{src.stem}_{stage}_spv"
            if name in seen:
                continue
...
def cmd_verify(args: argparse.Namespace) -> int:
    """Check that the manifest and the renderer agree on every variant name.

    A name the renderer asks for and the manifest does not build is a fatal
    error at startup; a variant the manifest builds and nobody asks for is dead
    weight in the pak. Both are cheap to detect here and expensive to notice at
    runtime, so this runs in CI.
    """
    _, variants = load_manifest(Path(args.manifest))
    available = {v.name for v in variants}

    source = Path(args.source).read_text(encoding="utf-8")
    source = source.replace("#define SHADER_MODULE( name ) vk_shader_module( #name )", "")
    used = set(re.findall(r"SHADER_MODULE\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)", source))
    used |= {v.name for v in variants if v.slot}

    missing = sorted(used - available)
    unused = sorted(available - used)

    print(f"referenced by the renderer : {len(used)}")
    print(f"produced by the manifest   : {len(available)}")

    if missing:
        print(f"\nFAILED: {len(missing)} referenced variant(s) are not built:")
        for name in missing[:20]:
            print(f"  {name}")
        return 1

    if unused:
        print(f"\n{len(unused)} variant(s) built but never referenced:")
        for name in unused[:20]:
            print(f"  {name}")
        if args.strict:
            return 1

    print("\nOK: manifest and renderer agree")
    return 0


def cmd_bind(args: argparse.Namespace) -> int:
    """Emit the slot -> variant-name table the renderer walks at startup.

    Upstream generated shader_binding.c, ~1400 lines of assignments into the
    multidimensional vk.shaders arrays, alongside the 61.7 MB blob array. Only
    the mapping is generated now; the bytes come from the pak.
    """
    _, variants = load_manifest(Path(args.manifest))
    bound = [v for v in variants if v.slot]

    lines = [
        "// Generated by tools/shadergen/shadergen.py -- do not edit.",
        "// JKX_SHADER_SLOT(slot expression, variant name)",
        "",
    ]
    for v in bound:
        lines.append(f'JKX_SHADER_SLOT(vk.shaders.{v.slot}, "{v.name}")')

    out = Path(args.out)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"bound {len(bound)} slot(s) -> {out}")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    p = sub.add_parser("verify", help="check manifest against renderer usage")
    p.add_argument("--source", default="code/rd-vulkan/vk_shaders.cpp")
    p.add_argument("--strict", action="store_true", help="also fail on unused variants")
    p.set_defaults(func=cmd_verify)

    p = sub.add_parser("bind", help="emit the slot table")
    p.add_argument("--out", required=True)
