...
static inline wireframeMapSurf_t *R_GetNewWireframeMapSurf( void )
{
	...
	(*next) = (wireframeMapSurf_t *)R_Z_Malloc(sizeof(wireframeMapSurf_t), TAG_ALL, qtrue);
...
static inline void R_EvaluateWireframeSurf( msurface_t *surf )
{
	if (*surf->data == SF_FACE)
	{
		...
		{ //we can add it
			...
			nextSurf->points = (wireframeSurfPoint_t *)R_Z_Malloc(sizeof(wireframeSurfPoint_t)*face->numIndices, TAG_ALL, qtrue);
...
qboolean R_WriteWireframeMapToFile( void )
{
    ...
    out = (byte *)R_Z_Malloc(requiredSize, TAG_ALL, qtrue);
	rOut = out;
...
qboolean R_GetWireframeMapFromFile( void )
{
	...
	while (i < len)
	{
		...
		newSurf->points = (wireframeSurfPoint_t *)R_Z_Malloc(sizeof(wireframeSurfPoint_t)*surfs->numPoints, TAG_ALL, qtrue);
