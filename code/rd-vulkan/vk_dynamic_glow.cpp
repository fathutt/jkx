...
qboolean vk_begin_dglow_blur( void )
{
	...
	for ( i = 0; i < VK_NUM_BLUR_PASSES * 2; i += 2 ) {
		...
		vkCmdBindPipeline( vk.cmd->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.dglow_blur_pipeline[i + 0] );
		vkCmdBindDescriptorSets( vk.cmd->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.pipeline_layout_post_process, 0, 1, &vk.dglow_image_descriptor[i + 0], 0, NULL );
		...
		vkCmdBindPipeline( vk.cmd->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.dglow_blur_pipeline[i + 1] );
		vkCmdBindDescriptorSets( vk.cmd->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.pipeline_layout_post_process, 0, 1, &vk.dglow_image_descriptor[i + 1], 0, NULL );
		vkCmdDraw( vk.cmd->command_buffer, 4, 1, 0, 0 );
		vk_end_render_pass();
	...
	vk_begin_post_blend_render_pass( vk.render_pass.dglow.blend, qtrue );
	{
		...
		vkCmdBindPipeline( vk.cmd->command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vk.dglow_blend_pipeline );
