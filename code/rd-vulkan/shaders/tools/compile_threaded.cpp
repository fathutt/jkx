...
static void compile_and_convert_template_shaders( void )
{
    ...
    const char* fastlight_flags[]   = { "", "-DUSE_FAST_LIGHT", };
    const char* fastlight_ids[]     = { "", "fastlight_" };

    const char* light_flags[]       = { "", "-DUSE_LIGHTMAP", "-DUSE_LIGHT_VECTOR", "-DUSE_LIGHT_VERTEX"};
    const char* light_ids[]         = { "", "lightmap_", "vector_", "vertex_" };
    ...
    create_shader_task("gen_frag.tmpl", "frag", "frag_tx0_df", NULL, "-DUSE_ATEST -DUSE_DF");
    ...
    for ( i = 0; i < ARRAY_LEN(vbo_flags); ++i ) { // vbo
        ...
        for ( n = 0; n < ARRAY_LEN(fastlight_flags); ++n ) { // fastlight
            for ( o = 0; o < ARRAY_LEN(light_flags); ++o ) { // light
                for ( j = 0; j < ARRAY_LEN(tx_flags); ++j ) { // tx
                    for ( k = 0; k < ARRAY_LEN(env_flags); ++k ) { // env
                        for ( l = 0; l < ARRAY_LEN(fog_flags); ++l ) { // fog
                            defines = join_flags({ vbo_flags[i], fastlight_flags[n], light_flags[o], tx_flags[j], env_flags[k], fog_flags[l] });
                            name    = "vert_" + std::string(vbo_ids[i]) + fastlight_ids[n] + light_ids[o] + tx_ids[j] + env_ids[k] + fog_ids[l];
                            ids     = join_indexes("vk.shaders.vert.gen", { i, n, o, j, 0, k, l });

                            create_shader_task("gen_vert.tmpl", "vert", name.c_str(), ids.c_str(), defines.c_str());

                            if ( j != 0 ) // tx
                            { 
                                defines_cl = join_flags({ vbo_flags[i], fastlight_flags[n], light_flags[o], tx_flags[j], cl_flags[j], env_flags[k], fog_flags[l] });
                                name_cl    = "vert_" + std::string(vbo_ids[i]) + fastlight_ids[n] + light_ids[o] + tx_ids[j] + "_" + cl_ids[j] + env_ids[k] + fog_ids[l];
                                ids_cl     = join_indexes("vk.shaders.vert.gen", { i, n, o, j, 1, k, l });
        ...
        for ( o = 0; o < ARRAY_LEN(light_flags); ++o ) { // light
            for ( j = 0; j < ARRAY_LEN(tx_flags); ++j) { // tx
                for ( k = 0; k < ARRAY_LEN(fog_flags); ++k ) { // fog
                    defines = join_flags({ fastlight_flags[n], light_flags[o], tx_flags[j], fog_flags[k] });

                    if ( j == 0 ) 
                        defines += " -DUSE_ATEST";

                    name    = "frag_" + std::string(fastlight_ids[n]) + light_ids[o] + tx_ids[j] + fog_ids[k];
                    ids     = join_indexes("vk.shaders.frag.gen", { n, o, j, 0, k });

                    create_shader_task("gen_frag.tmpl", "frag", name.c_str(), ids.c_str(), defines.c_str());

                    if ( j != 0 ) // tx
                    { 
                        defines_cl  = join_flags({ fastlight_flags[n], light_flags[o], tx_flags[j], cl_flags[j], fog_flags[k] });
                        name_cl     = "frag_" + std::string(fastlight_ids[n]) + light_ids[o] + tx_ids[j] + "_" + cl_ids[j] + fog_ids[k];
                        ids_cl      = join_indexes("vk.shaders.frag.gen", { n, o, j, 1, k });
