...
static CRenderableSurface *AllocGhoul2RenderableSurface( void )
{
	if ( currentRenderSurfIndex >= MAX_RENDERABLE_SURFACES )
	{
		...
		Com_Error( ERR_DROP, "AllocRenderableSurface: Reached maximum number of Ghoul2 renderable surfaces (%d)", MAX_RENDERABLE_SURFACES );
		return NULL;
	}
...
static int G2_ComputeLOD( trRefEntity_t *ent, const model_t *currentModel, int lodBias )
{
	...
	//CL_RefPrintf(PRINT_ALL, "radius: %f lod: %d: flod: %f dist: %f numLODS: %d\n", radius, lod, flod, dist, currentModel->numLods);
	return lod;
}
...
void G2_RagGetAnimMatrix(CGhoul2Info &ghoul2, const int boneNum, mdxaBone_t &matrix, const int frame)
{
	...
	else
	{
		...
		{
#ifdef _RAG_PRINT_TEST
			CL_RefPrintf( PRINT_ALL, "Attempting to add %s\n", skel->name);
#endif
	...
	if (boneNum > 0 && parent > -1)
	{
		...
		{
			CL_RefPrintf( PRINT_ALL, "BAD LIST INDEX: %s, %s [%i]\n", skel->name, pskel->name, parent);
		}
#endif
	}
	else
	{ //root
		...
		{
			CL_RefPrintf( PRINT_ALL, "BAD LIST INDEX: %s\n", skel->name);
		}
#endif
	...
	if (!actuallySet)
	{
		CL_RefPrintf( PRINT_ALL, "SET FAILURE\n");
...
qboolean R_LoadMDXM( model_t *mod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) {
	...
	if (version != MDXM_VERSION) {
		CL_RefPrintf( PRINT_ALL, S_COLOR_YELLOW  "R_LoadMDXM: %s has wrong version (%i should be %i)\n",
	...
	mod->data.glm = (mdxmData_t *)R_Hunk_Alloc(sizeof (mdxmData_t), h_low);
...
//		memcpy( mdxm, buffer, size );	// and don't do this now, since it's the same thing
	...
	if (!mdxm->animIndex)
	{
		CL_RefPrintf( PRINT_ALL, S_COLOR_YELLOW  "R_LoadMDXM: missing animation file %s for mesh %s\n", mdxm->animName, mdxm->name);
		return qfalse;
	}
	...
	// Is this an Outcast mesh on an Academy skeleton? See tr_ghoul2_bonemap.h -
	// the question is asked of the two bone counts, not of the skeleton's name.
	// The skeleton is loaded: animIndex was registered above and checked.
	int skeletonBones = 0;
	{
		const model_t *mod_a = R_GetModelByHandle( mdxm->animIndex );

		if ( mod_a && mod_a->type == MOD_MDXA && mod_a->data.gla ) {
			skeletonBones = mod_a->data.gla->numBones;
		}
	}
	...
	for ( l = 0 ; l < mdxm->numLODs ; l++)
	{
		...
		for ( i = 0 ; i < mdxm->numSurfaces ; i++)
		{
			...
			if ( bRemapBones )
			{
				...
				for ( j = 0 ; j < surf->numBoneReferences ; j++ )
				{
					// No assert here. The index came out of a file, so an index
					// this table cannot translate is a statement about the file
...
qboolean R_LoadMDXA( model_t *mod, void *buffer, const char *mod_name, qboolean &bAlreadyCached ) {
	...
	if (version != MDXA_VERSION) {
		CL_RefPrintf( PRINT_ALL, S_COLOR_YELLOW  "R_LoadMDXA: %s has wrong version (%i should be %i)\n",
...
//		memcpy( mdxa, buffer, size );	// and don't do this now, since it's the same thing
	...
	if (!bAlreadyFound)
	{
 	...
 	if ( mdxa->numFrames < 1 ) {
		CL_RefPrintf( PRINT_ALL, S_COLOR_YELLOW  "R_LoadMDXA: %s has no frames\n", mod_name );
		return qfalse;
	}
