...
void RE_StretchRaw ( int x, int y, int w, int h, int cols, int rows, const byte *data, int client, qboolean dirty )
{
	...
	start = 0;
	if (r_speeds->integer) {
		start = Sys_Milliseconds2() * Cvar_VariableValue("timescale");
	}
	...
	if (r_speeds->integer) {
		end = Sys_Milliseconds2() * Cvar_VariableValue("timescale");
		CL_RefPrintf(PRINT_ALL, "RE_UploadCinematic( %i, %i ): %i msec\n", cols, rows, end - start);
	}
...
void RB_ExecuteRenderCommands( const void *data ) {
	...
	t1 = Sys_Milliseconds2()*Cvar_VariableValue( "timescale" );
	...
	while ( 1 ) {
		...
		default:
			// stop rendering
			vk_end_frame();
			t2 = Sys_Milliseconds2()*Cvar_VariableValue( "timescale" );
