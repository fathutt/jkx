...
void R_BuildSurfaceSpritesVBO( const world_t &worldData, int index ) 
{
	...
	sprite_t *sprite_instances = (sprite_t *)R_Hunk_AllocateTempMemory( sizeof(sprite_t) * estimate_num_instances );
	spriteStage_t **sprites_surf = (spriteStage_t **)R_Hunk_AllocateTempMemory( sizeof(spriteStage_t *) * estimate_num_surfs );
	...
	for ( i = 0; i < worldData.numsurfaces; ++i )
	{
		...
		surf->surface_sprites.stage = (spriteStage_t *)R_Hunk_Alloc( sizeof(spriteStage_t) * surf->surface_sprites.num_stages, h_low );
	...
	R_Hunk_FreeTempMemory( sprite_instances );
	R_Hunk_FreeTempMemory( sprites_surf );
}
