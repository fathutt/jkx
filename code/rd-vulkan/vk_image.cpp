...
void vk_destroy_samplers( void )
{
	...
	for ( i = 0; i < vk.samplers.count; i++ ) {
		vkDestroySampler( vk.device, vk.samplers.handle[i], NULL );
...
VkSampler vk_find_sampler( const Vk_Sampler_Def *def ) {
	...
	VK_CHECK(vkCreateSampler(vk.device, &desc, NULL, &sampler));
	...
	VkImageLayout old_layout, VkImageLayout new_layout, uint32_t src_stage_override, uint32_t dst_stage_override )
{
	...
	switch ( old_layout ) {
		...
		case VK_IMAGE_LAYOUT_GENERAL:
			...
			break;
		// The two depth layouts. Needed since the depth buffer became something
		// that can be read as a texture between passes - see vk_depth_resolve.
		// The wait is on LATE fragment tests because that is where the last
		// write to a depth attachment happens, and on the whole fragment stage
		// on the way back because a shader that has just read it may still be
		// running.
...
static void vk_clear_cube_color( image_t *image, VkClearColorValue color ) 
{
	...
	vkCmdClearColorImage( command_buffer, image->handle, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &color, 1, &desc );	
...
static void vk_upload_cube( image_t *image ) {
	...
	const uint32_t numMips = static_cast<uint32_t>(floor(log2(image->width))) + 1;

	vk_create_image( image, image->width, image->height, numMips );
	vk_clear_cube_color( image, { 1.0, 1.0, 1.0, 1.0 } );
}
...
void vk_flush_staging_buffer( qboolean final )
{
	...
	VK_CHECK( vkEndCommandBuffer( vk.staging_command_buffer ) );
...
void vk_create_image( image_t *image, int width, int height, int mip_levels ) {
	...
	// create image view
	{
		...
		VK_CHECK( vkCreateImageView( vk.device, &desc, NULL, &image->view ) );
	}
...
static void R_CreateDlightImage( void )
{
    ...
    {	// if we dont get a successful load
        ...
        tr.dlightImage = R_CreateImage("*dlight", (byte*)data, DLIGHT_SIZE, DLIGHT_SIZE, IMGFLAG_CLAMPTOEDGE, 0, 0);
    }
}
...
static void R_CreateBuiltinImages( void ) {
	...
	tr.whiteImage = R_CreateImage("*white", (byte*)data, 8, 8, IMGFLAG_NONE, 0, 0);

	Com_Memset(data, 0, sizeof(data));
	tr.blackImage = R_CreateImage("*black", (byte*)data, 8, 8, IMGFLAG_NONE, 0, 0);
	...
	tr.identityLightImage = R_CreateImage("*identityLight", (byte*)data, 8, 8, IMGFLAG_NONE, 0, 0);
