...
CTRLandScape::CTRLandScape(const char *configstring)
{
	...
	mRenderMap = (CTerVert *)R_Z_Malloc(sizeof(CTerVert) * common->GetRealArea(), TAG_R_TERRAIN);
	...
	mTRPatches = (CTRPatch *)R_Z_Malloc(sizeof(CTRPatch) * common->GetBlockCount(), TAG_R_TERRAIN);

	mSortedCount = 2 * common->GetBlockCount();
	mSortedPatches = (TPatchInfo *)R_Z_Malloc(sizeof(TPatchInfo) * mSortedCount, TAG_R_TERRAIN);
