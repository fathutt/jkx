...
// There used to be a second copy of the whole list here, every entry suffixed
// _JK2, in Jedi Outcast's order - and a 360-line switch in cl_cgame.cpp that
// translated one into the other on every system call the JK2 cgame made.
//
// Two ordinal lists of the same 112 calls, kept in step by hand, with nothing
// that would fail if they ever drifted apart. They had not drifted; checked
// entry by entry before removing it. That is luck, not a property.
//
...
// games/jo now builds against this list, so there is only one set of numbers.
