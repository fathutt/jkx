...
// refimport_t stood here: about a hundred function pointers the client filled
// in and handed to the renderer at load time, because the renderer was a
// separate module and had no other way to reach the engine. There is one
// binary now. The renderer calls the engine directly, and what it may call is
// declared in code/rd-vulkan/tr_engine_api.h - where the compiler checks the
// signatures, the calls can be inlined, and a backtrace has function names in
// it instead of pointers.
...
typedef	refexport_t* (QDECL *GetRefAPI_t) (int apiVersion);
