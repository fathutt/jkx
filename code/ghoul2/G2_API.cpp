...
void G2_DEBUG_ReportLeaks(void)
{
	...
	{
		CL_RefPrintf( PRINT_ALL, "g2 leak tracker was never initialized!\n");
		return;
	}
    ...
    while (i < MAX_TRACKED_ALLOC)
	{
		if (g_G2AllocTrack[i])
		{
			CL_RefPrintf( PRINT_ALL, "Bad guy found in slot %i, attempting to access...", i);
			CGhoul2Info_v &g2v = *g_G2AllocTrack[i];
			CGhoul2Info &g2 = g2v[0];

			if (g2v.IsValid() && g2.mFileName && g2.mFileName[0])
			{
				CL_RefPrintf( PRINT_ALL, "Bad guy's filename is %s\n", g2.mFileName);
			}
			else
			{
				CL_RefPrintf( PRINT_ALL, "He's not valid! BURN HIM!\n");
			}
		}
		i++;
	}
...
void G2_DEBUG_ShovePtrInTracker(CGhoul2Info_v *g2)
{
	...
	{
		CL_RefPrintf( PRINT_ALL, "Crap g2 instance passed to instance tracker (in).\n");
	}
}
...
void G2_DEBUG_RemovePtrFromTracker(CGhoul2Info_v *g2)
{
	...
	{
		CL_RefPrintf( PRINT_ALL, "Crap g2 instance passed to instance tracker (out).\n");
	}
}
#endif
...
#if G2_DEBUG_TIME
	CL_RefPrintf( PRINT_ALL, "Set Time: before c%6d  s%6d",G2TimeBases[1],G2TimeBases[0]);
#endif
...
#if G2_DEBUG_TIME
	CL_RefPrintf( PRINT_ALL, " after c%6d  s%6d\n",G2TimeBases[1],G2TimeBases[0]);
#endif
}
...
char *G2API_GetAnimFileInternalNameIndex( qhandle_t modelIndex )
{
	...
	mdxaHeader_t *mdxa = R_GetGhoul2AnimHeader( R_GetModelByHandle( modelIndex ) );
...
{
	return R_GetGhoul2MeshHeader( R_GetModelByHandle(modelIndex) )->animName;
}
...
// The trace type is an EG2_Collision, not a bare int - multiplayer spelled it
// as an int and lost that at every call site.
void G2API_CollisionDetect(CollisionRecord_t *collRecMap, CGhoul2Info_v &ghoul2, const vec3_t angles, const vec3_t position,
										  int frameNumber, int entNum, vec3_t rayStart, vec3_t rayEnd, vec3_t scale, IHeapAllocator *G2VertSpace, EG2_Collision traceFlags, int useLod, float fRadius)
{
	/*
	if (1)
	{
...
char *G2API_GetSurfaceName(CGhoul2Info *ghlInfo, int surfNumber)
{
	...
	if (G2_SetupModelPointers(ghlInfo))
	{
		...
		{
			CL_RefPrintf( PRINT_ALL, "G2API_GetSurfaceName: You passed in an invalid surface number (%i) for model %s.\n", surfNumber, ghlInfo->mFileName);
			return noSurface;
		}
...
qboolean G2_TestModelPointers(CGhoul2Info *ghlInfo) // returns true if the model is properly set up
{
	...
	if (ghlInfo->mModelindex != -1)
	{
		...
		{
			mdxmHeader_t *mdxm = R_GetGhoul2MeshHeader( ghlInfo->currentModel );
			if ( mdxm )
			{
